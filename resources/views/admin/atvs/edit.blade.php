                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.atv.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group" id="field_sction">
                                ...
                                @php
                                    $count = 0;
                                    dd(json_decode($atv->price))
                                @endphp
                                @foreach (json_decode($atv->price) as $price_hour);
                                {{dd($price_hour)}}
                                    <div class="row">
                                        <div class="col-4">
                                            <input type="number" name="hour[]" placeholder="Hour"
                                                value="{{ str_replace('hour:', '', $price_hour) }}" id=""
                                                class="form-control">
                                        </div>
                                        <div class="col-4">
                                            <input type="number" name="hour[]" placeholder="Price"
                                                value="{{ str_replace('price:', '', $price_hour) }}" id=""
                                                class="form-control">
                                        </div>
                                        <div class="col-4">
                                            <button type="button" class="btn btn-primary" onclick="add_field()"><i
                                                    class="fa fa-plus"></i></button>
                                            {{-- <button type="button" class="btn btn-danger" onclick="delete_append_sec()"><i class="fa fa-trash"></i></button> --}}
                                        </div>
                                    </div><br>
                                    @php
                                        $count++;
